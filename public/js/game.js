...
// ── Dice Panel / Action Buttons ───────────────────────────────
function renderDicePanel() {
  if (!gameState) return;
  const rollBtn   = document.getElementById('btn-roll');
  const endBtn    = document.getElementById('btn-end-turn');
  const debtBtn   = document.getElementById('btn-resolve-debt');
  
  // Show manual bankrupt button if playing
  const bankruptBtn = document.getElementById('btn-bankrupt-manual');
  if (bankruptBtn) {
    const me = gameState.players.find(p => p.id === myPlayerId);
    if (me && !me.isBankrupt && !gameState.gameOver) {
  ...
  if (iAmCurrentPlayer) {
    if (gameState.phase === 'rolling') {
      ...
      if (debtBtn) debtBtn.classList.add('hidden');
      ...
      if (debtBtn) {
        ...
        const me = gameState.players.find(p => p.id === myPlayerId);
        // Only allow resolving if money >= 0
        debtBtn.disabled = me && me.money < 0;
        debtBtn.style.opacity = debtBtn.disabled ? '0.5' : '1';
      }
    } else {
      rollBtn.classList.add('hidden');
      endBtn.classList.add('hidden');
...
function showPropInfo(propId) {
  ...
  if (me && iAmCurrentPlayer) {
    // You can mortgage/unmortgage during rolling, endturn, or debt phases
    if (['rolling', 'endturn', 'debt'].includes(gameState.phase)) {
      if (me.properties.includes(propId)) {
        if (me.mortgaged && me.mortgaged.includes(propId)) {
...
window.resolveDebt = function() {
  if (!iAmCurrentPlayer) return;
  socket.emit('resolve-debt', { roomCode: myRoomCode, playerId: myPlayerId }, (res) => {
    if (res?.error) showToast(res.error, 'error');
  });
};

window.declareBankrupt = function() {
  if (!confirm(lang === 'az' ? 'Həqiqətən oyundan çıxmaq və müflis olmaq istəyirsiniz?' : 'Are you sure you want to go bankrupt and leave the game?')) return;
  socket.emit('declare-bankrupt', { roomCode: myRoomCode, playerId: myPlayerId }, (res) => {
    if (res?.error) showToast(res.error, 'error');
  });
};
